...
def add_options():
  ...
  flags.DEFINE_integer('port', default = 8082, help = 'port number')
...
def main(unused_argv):
  ...
  prompt = cypher_generation_template(tokenizer, neo4j, entity_types)
  chain = prompt | llm
  ...
  with block as demo:
    with gr.Row(equal_height = True):
      with gr.Column(scale = 15):
        gr.Markdown("<h1><center>cypher命令生成</center></h1>")
    with gr.Row():
